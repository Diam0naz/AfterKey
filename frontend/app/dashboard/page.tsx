      ...
      try {
        ...
        const data = Array.isArray(res) 
          ...
          : {
              ...
              required: Number(res.required_approvals ?? 0),
              deadline: Number(res.execution_deadline ?? 0),
            };

        setStatus(data);
      } catch (err) {
        console.error("Vault data sync failed:", err);
      } finally {
        setLoading(false);
      }
    }
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Logged in as: <span className="text-indigo-400">{user?.email?.address}</span></p>
        </div>
        <WalletAddress />
      </div>
      {}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PLANS.map((plan) => (
            <PlanCard 
              key={plan.name} 
              {...plan} 
              onActivate={() => console.log(`Activating ${plan.name} plan...`)} 
            />
          ))}
        </div>
      </div>
